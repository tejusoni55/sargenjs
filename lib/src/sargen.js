...
class Builder {
   ...
   * @param {boolean} options.db - Whether to setup database configuration
  ...
  _initializeProject(options) {
    try {
      ...
      fileHelper._createReadmeFile(this.projectPath, {
        ...
        hasDatabase: dbConf,
        databaseType: dbConf ? "mysql" : null, // Default to mysql if db is enabled
        hasTest: addTest,
      });
      ...
      // Setup database configuration if requested
      if (dbConf) {
