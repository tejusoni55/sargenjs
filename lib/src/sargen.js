...
class Generate {
  ...
  _generateMigrationContent(moduleName, modelAttributes) {
    ...
    const attributesString = modelAttributes.map(attr => {
      let attrString = `      ${attr.name}: {
        type: Sequelize.${attr.type},
        allowNull: false`;
      
      // Add foreign key constraints if it's a foreign key
...
${attributesString},
  ...
  _generateModule(moduleName, options = {}) {
    try {
      ...
      if (options.model && modelAttributes && modelAttributes.length > 0) {
        ...
        logger().info("   Note: for modular structure, Run: npx sequelize-cli db:migrate --migrations-path common/migrations");
   ...
   * @param {Object} options - Options for util generation
   * @param {boolean} options.docker - Whether to add docker configuration
   * @param {string} options.cloudProvider - Cloud provider for fileupload (s3, gcp, or null)
   * @param {boolean} options.force - Whether to overwrite existing fileupload service file
   */
...
class Setup {
  ...
  _validateNodeProject() {
    ...
    try {
      ...
      const recommendedDeps = ['dotenv', 'helmet', 'cors'];
