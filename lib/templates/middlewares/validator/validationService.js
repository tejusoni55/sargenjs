...
class ValidationService {
  ...
  validateQuery(schemaName, options = {}) {
    return (req, res, next) => {
      ...
      req.params = { ...req.body };
      const result = schema(req.params, options);

      if (result === true) {
        req.validatedQuery = req.params;
        next();
      } else {
        return res.status(400).json({
          success: false,
...
valdiationService.validateQuery =
  valdiationService.validateQuery.bind(valdiationService);
