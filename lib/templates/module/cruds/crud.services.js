...
create<%= moduleName %>: async function (data) {
    try {
...
get<%= moduleName %>: async function (query = {}) {
    try {
        <% if (modelAttributes && modelAttributes.length > 0) { %>
        // Get all records from the database
        const records = await <%= moduleName %>Model.findAll();
        return {
            success: true,
...
update<%= moduleName %>: async function (id, data) {
    try {
        <% if (modelAttributes && modelAttributes.length > 0) { %>
...
delete<%= moduleName %>: async function (id) {
    try {
        <% if (modelAttributes && modelAttributes.length > 0) { %>
