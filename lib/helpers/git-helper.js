...
const gitHelper = {
  ...
  async _createGitHubRepository(repoName, options = {}, verbose = false) {
    try {
      ...
      const createResult = await cliHelper._runCommand(createCommand, { verbose: true });
  ...
  _createGitignore(projectPath) {
    try {
      ...
      // Read the template
      const templatePath = path.join(__dirname, "../templates/git/.gitignore");
      ...
      try {
        ...
        if (remoteUrl) {
          ...
          if (!options.noPush) {
            ...
            // Try to push with more verbose output
            const pushResult = await cliHelper._runCommand(`git push -u origin ${targetBranch}`, { verbose: true });
            if (!pushResult.success) {
              ...
              logger().info(`Repository is now available at: ${options.remote}`);
            }
          } else {
